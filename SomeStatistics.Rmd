...
trinotate <- filter(trinotate, !is.na(prot_id)) %>% left_join(longitudes) %>% separate(prot_coordinates, sep = "-", into = c("prot_begin", "prot_end")) %>% mutate(prot_length = abs(as.integer(prot_begin) - as.integer(prot_end)))
...
scatterplot <- left_join(trinotate, longitudes) %>% filter(peptide_type == "Swissprot") %>% ggplot() + geom_point(aes(log(total_length), log(prot_length))) + theme_bw() + labs(x = "log of mRNA length", y = "log of ORF length")

xdensity <- left_join(trinotate, longitudes) %>% filter(peptide_type == "Swissprot") %>%  ggplot(aes(log(total_length))) + geom_density() + theme_bw() + labs(title = "length of mRNA vs the length of the transdecoder predicted ORF (nucleotides)")

ydensity <- left_join(trinotate, longitudes) %>% filter(peptide_type == "Swissprot") %>% ggplot(aes(log(prot_length))) + geom_density() + theme_bw() + coord_flip()
